...
| IMU           | ICM-20602     | 加速度 + 角速度（姿态/振动）   | SPI      | 
| GPS           | NEO-M8N       | 经纬度、速度、高度、时间       | UART     | 
| 气压高度计    | BMP388        | 高精度相对高度（抗 GPS 漂移）  | I²C      | 
| 4G 模块       | SIM7600CE-T   | LTE 数据回传                   | UART     | 
| 存储          | MicroSD 卡    | 数据黑匣子（CSV 格式）         | SPI      | 
| 电源          | 3.7–5V LiPo   | 供电（SIM7600 需大电流）       | —        | 
